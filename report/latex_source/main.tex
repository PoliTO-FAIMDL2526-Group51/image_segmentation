...
\begin{abstract}
Safe autonomous driving requires efficient anomaly detection and semantic segmentation. In our work, we investigate the potential of the EoMT (Every Object is a Mask Token) framework in relation to its cross-domain and fine-tuning properties. Specifically, we evaluate the transferability of an EoMT pre-trained on the COCO panoptic segmentation task on the Cityscapes validation set. To overcome possible domain discrepancy, we propose two fine-tuning strategies, namely head-only and last-block fine-tuning. Moreover, we investigate how well EoMT models can perform out-of-distribution (OOD) anomaly detection using multiple post-hoc uncertainty estimates based on maximum logit, max entropy, and region-based aggregation (RbA). In comparison with a generic pixel-based architecture like ERFNet, we show that the usage of the EoMT framework along with the max entropy score is extremely encouraging to detect dangerous anomalies. Our code is available: \url{https://github.com/PoliTO-FAIMDL2526-Group51/image_segmentation}.
\end{abstract}
...
Autonomous driving needs reliable road-scene understanding, and segmentation is one of the main tools for this because it assigns labels to image regions at pixel level. At the beginning of the project, we reviewed several segmentation tasks and models. ERFNet~\cite{romera2017erfnet} was used as an example of an efficient semantic segmentation model for real-time applications, while Panoptic Segmentation~\cite{kirillov2019panoptic} introduced the idea of combining semantic labels with instance-level information. We then studied mask-based methods, starting from MaskFormer~\cite{cheng2021maskformer} and Mask2Former~\cite{cheng2022mask2former}, which treat segmentation more as a mask classification problem. DINOv2~\cite{oquab2023dinov2} and EoMT~\cite{eomt2025} were also relevant because they show how strong ViT features can be used for segmentation. Based on this background, Step 4 compares two pretrained EoMT models: one trained on Cityscapes for semantic segmentation and one trained on COCO for panoptic segmentation. Since their tasks and label spaces are different, we mapped their predictions to a common semantic label space and evaluated them on the Cityscapes validation set using mIoU. In step 5 we have analyzed how well can COCO EoMT model perform when fine-tuned with Cityscapes. We used two different lightweight approaches, head-only and last-block fine-tuning.
...
Finally, we evaluate the mask-based EoMT checkpoints for anomaly segmentation in open-world road scenes. We apply MSP, MaxLogit, MaxEntropy, and the mask-specific RbA score to detect pixels outside the known road-scene distribution. We also study how checkpoint training and temperature scaling affect anomaly detection across five benchmark datasets.


\section{Methodology}


As required in step 4, we compares the two provided EoMT checkpoints on the Cityscapes validation set. The two models are:
...
The final metric is mean Intersection over Union (mIoU), computed on the full Cityscapes validation set. \cref{fig:step4_comparison} is a example.
...
Two performance metrics were used for each method across the different datasets, keeping the same preprocessing and evaluation pipeline: AuPRC, Area under the Precision-Recall Curve, and FPR95, False Positive Rate at 95\% True Positive Rate.
...
\end{equation}
excluding the no-object class. From $\mathbf{p}(i)=\softmax(\mathbf{S}(i))$, we
...
$\softmax(\mathbf{z}_q)$ in \cref{eq:aggregation} with
$\softmax(\mathbf{z}_q/T)$ for
$T\in\{0.25,0.5,0.75,1,1.5,2,2.5\}$.

We evaluate EoMT trained on COCO and Cityscapes, plus the Step 5 head-only and
last-block fine-tuned checkpoints. All four scores are evaluated on SMIYC RA-21,
SMIYC RO-21, FS Lost \& Found, FS Static, and Road Anomaly.
...
\multirow{4}{*}{EoMT FT-LastBlock}
    & MSP & 0.81 & 62.1/24.9  & 94.2/0.3 & 35.3/19.1 & 86.0/4.7 & 67.1/40.0 \\
    & MaxLogit && 61.5/24.3  & 94.2/0.3 & 35.6/17.9 & 86.0/4.5 & 64.7/81.2 \\
    & MaxEntropy && 63.1/23.8  & \textbf{94.5/0.3} & 37.2/17.4 & \textbf{86.3}/3.9 & 65.2/81.4 \\
    & RbA && 58.9/19.0  & 92.2/0.4 & 36.5/78.6 & 84.4/\textbf{3.8} & 49.1/98.4 \\
\bottomrule
\end{tabular}}
\end{table*}

The raw COCO checkpoint is consistently the weakest anomaly detector because its broad known-class vocabulary includes many objects considered anomalous in road scenes. Adapting the model to Cityscapes greatly improves anomaly detection, but there is no universally best fine-tuning depth: FT-Head, FT-LastBlock, and the Cityscapes checkpoint each perform best on different datasets. Among the post-hoc scores, MaxEntropy is the most reliable default, whereas RbA is substantially less stable across datasets and operating points. Temperature scaling can improve ranking, but its effect depends on the checkpoint, dataset, score, and target metric.
...
\begin{tabular}{llccc}
\toprule
Checkpoint & Dataset & $T{=}1$ AP & Best $T$ & Best AP \\
\midrule
...
\section{Discussion}
\subsection{Comparison of two model}
...
This can be caused by the resolution of images, limited epochs due to the need for more compute power, and COCO Cityscapes differences. Since our experimentation plays a role in exploring how close we can get by basic and cheap methods, further trials might have some more promising results.
...
\bibitem{romera2017erfnet}
E. Romera, J. M. Alvarez, L. M. Bergasa, and R. Arroyo,
``ERFNet: Efficient Residual Factorized ConvNet for Real-Time Semantic Segmentation,''
\emph{IEEE Transactions on Intelligent Transportation Systems}, 2017.
